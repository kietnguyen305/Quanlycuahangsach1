<?php
	if (!function_exists("db_connect")){
		...
		function db_connect(){
			...
			if(!$conn){
				echo "Không thể kết nối cơ sở dữ liệu " . mysqli_connect_error($conn);
				exit;
			}
			return $conn;
		}
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT book_id, img FROM books ORDER BY book_id DESC";
...
if (!function_exists("getBookByIsbn")){
	function getBookByIsbn($conn, $isbn){
		$query = "SELECT book_author, book_price FROM book WHERE book_id = '$id'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Khoogn thể truy xuất dữ liệu " . mysqli_error($conn);
			exit;
		}
		return $result;
	}
...
if (!function_exists("insertIntoCart")){
	function insertIntoCart($conn, $customerid,$date){
		...
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Thêm vào giỏ hàng không thành công " . mysqli_error($conn);
			exit;
		}
	}
}
if (!function_exists("getbookprice")){
	function getbookprice($isbn){
		...
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Hiển thị giá không thành công! " . mysqli_error($conn);
			exit;
		}
		$row = mysqli_fetch_assoc($result);
